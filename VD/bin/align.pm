...
sub filter_SAM
{
	...
	my $kept_align = 0;
        ...
        $in->close;
        $out->close;
        Util::process_cmd("move $temp_SAM $input_SAM");
...
sub pileup_filter
{
	...
	if ($virus_seq_info =~ m/\.gz$/) {
		open ($fh1, "$bin_dir/7za x $virus_seq_info -so |")|| die $!;
	} else {
...
sub remove_redundancy
{
	...
	if ($data_type eq 'mRNA') {
		...
		Util::process_cmd("move $cluster $contig_file", $debug);
...
sub base_correction
{
	...
	# Util::process_cmd("bowtie2 --quiet --end-to-end -D 20 -R 3 -N 0 -L 13 -i S,1,0.50 --gbar 1 -p $cpu_num $format -a -x $contig_file -U $read_file -S $read_file.sam", $debug);
        ...
        Util::process_cmd("$bin_dir/samtools view -@ $cpu_num -bS $read_file.sam > $read_file.bam 2> $temp_dir/samtools.log");
