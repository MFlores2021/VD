...
';
	...
	my $prefix1 = dirname($$files[0]);
	my $report_file = $prefix1 .'\\report_sRNA_trim.txt';
...

	{
		...
		#$out2->close;
		#$inFile =~ s/$prefix1/1/ee;

		$report_info.=basename($inFile)."\t$total_num\t".
	...
	my $out_len = IO::File->new(">$prefix1\\sRNA_length.txt") || die $!;
