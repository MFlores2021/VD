...
$datax->read(file => 'sRNA_length.txt', delimiter => '\t');
#$datax = $datax->copy(wanted => [0,1]);
print $datax->num_sets();
...
$graph->set_title_font(arial);
  $graph->plot($datax) or die $graph->error;

  my $file = catfile($i.'_spike.png');
